...
┌─────────────────────────────────────────────────────────────┐
│                        数据预处理层                           │
│  data_preprocessing.py ── Tokenizer分词 / 滑动窗口采样        │
│  config.py / config_clean.py ── 模型配置参数                  │
└──────────────────────────┬──────────────────────────────────┘
                           ▼
┌─────────────────────────────────────────────────────────────┐
│                      Transformer核心层                       │
│  attention.py ── 多头自注意力 / 因果掩码 / 前馈网络           │
│  gpt_model.py ── 12层Transformer解码器 / 文本生成            │
└──────────────────────────┬──────────────────────────────────┘
                           ▼
┌─────────────────────────────────────────────────────────────┐
│                        训练微调层                            │
│  pretrain_trainer.py ── 无标签数据预训练                      │
│  classification_finetune.py ── SMS垃圾短信分类               │
│  instruction_finetune.py ── 指令微调                         │
└──────────────────────────┬──────────────────────────────────┘
                           ▼
┌─────────────────────────────────────────────────────────────┐
│                        参数高效微调层                         │
│  lora_adapter.py ── LoRA低秩适应 / 冻结原参数                │
│  sft_finetune.py ── SFT监督微调 / 损失掩码                  │
└──────────────────────────┬──────────────────────────────────┘
                           ▼
┌─────────────────────────────────────────────────────────────┐
│                        检索增强层                            │
│  RAG.py ── 文档分块 / 文本嵌入 / FAISS检索 / 提示词拼接     │
│  knowledge/ ── 知识库向量存储                               │
└──────────────────────────┬──────────────────────────────────┘
                           ▼
┌─────────────────────────────────────────────────────────────┐
│                        交互展示层                            │
│  web_demo.py ── Gradio界面 / 参数调节 / 可视化输出           │
│  gpt_download.py ── GPT-2预训练权重下载                     │
└─────────────────────────────────────────────────────────────┘
```

```mermaid
flowchart TD
    A[数据预处理层<br>data_preprocessing.py<br>config.py] --> B[Transformer核心层<br>attention.py<br>gpt_model.py]
    B --> C[训练微调层<br>pretrain_trainer.py<br>classification_finetune.py]
    C --> D[参数高效微调层<br>lora_adapter.py<br>sft_finetune.py]
    D --> E[检索增强层<br>RAG.py<br>knowledge/]
    E --> F[交互展示层<br>web_demo.py]
    
    style A fill:#e8f4fd,stroke:#3498db
    style B fill:#e8f8f5,stroke:#1abc9c
    style C fill:#fef9e7,stroke:#f39c12
    style D fill:#fdeced,stroke:#e74c3c
    style E fill:#f5eef8,stroke:#9b59b6
...
B --> C["🏋️ Pretraining<br/>Language Modeling"]
